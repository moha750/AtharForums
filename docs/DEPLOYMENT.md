...
## ٩. ما يجب اختباره يدويًا بعد النشر

أمور لا يكفي فيها البناء الناجح — جرّبها بحساب حقيقي:

| الاختبار | المتوقَّع |
|---|---|
| تسجيل دخول ببريد `@hrsd.gov.sa` | يصل الرابط ويعمل |
| تسجيل دخول ببريد خارجي | **يُرفض** برسالة واضحة |
| انضمام إلى منتدى، ثم انسحاب، ثم إعادة تقديم | تنجح الثلاثة |
| منتدى بـ«قبول تلقائي» | الانضمام يُقبل فورًا |
| فتح الموقع بلا تسجيل دخول قبل التدشين | الصفحة التشويقية فقط |
| `/rest/v1/profiles` من المتصفّح بلا جلسة | **لا شيء** |
| فعالية «للأعضاء فقط» بلا منتدى | يراها كل عضو معتمد |
| إيقاف حساب موظف (`is_active = false`) | لا يعود يقرأ ولا يكتب |
